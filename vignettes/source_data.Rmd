...
source_df |>
  head() |>
  knitr::kable(caption = "The first few rows of `source_df`")
```
...
One benefit of the validation steps being embedded in the object itself is that these validations are automatically run when the object is modified. For example, this makes it impossible to modify the data later to an invalid object.

```{r}
#| eval: false

# for example, trying to modify the isotope will lead to an error
> source_object@data$isotope = "13G"

ERROR: Invalid isotope found: 13G
Error in qSIP2::validate_isotopes(dplyr::pull(self@data, isotope)) : 
...
source_df_MISIP |>
  ...
  head() |>
  knitr::kable(caption = "A 'MISIPified version of `source_df`")
```
...
source_df_MISIP |> count(isotope) |> knitr::kable(caption = "Count of `isotope` types in `source_df_MISIP`")
```
...
source_df_MISIP |> count(isotopolog_label) |> knitr::kable(caption = "Count of `isotopolog_label` types in `source_df_MISIP`")
```
...
source_df_ORIGINAL |>
  arrange(desc(total_copies_per_g)) |>
  head() |>
  knitr::kable(caption = "`source_df_MISIP` converted back")
```

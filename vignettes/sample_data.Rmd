...
Typically all of the `gradient_pos_rel_amt` for a given `source_mat_id` should add up to 1 (i.e. 100%), but there are situations where it might be less than 1 (although never be greater than 1). For example, if you removed some fractions because they didn't sequence well or there was some other reason to remove a fraction. If those removed fractions were 5% of the total data in that `source_mat_id`, then you would expect the total of all fractions to be 0.95 rather than 1. Another situation would be that although you added 25ng to a centrifugation, you only recovered 20ng and the rest was lost[^angel_share]. 

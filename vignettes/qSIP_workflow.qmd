...
For growth calculations there are three additional requirements: `timepoint`, `total_abundance` and `volume`. These are not necessary for the standard EAF workflow and will instead be addressed in the [growth vignette](vignettes/growth.html). 
...
See the [source data vignette](vignettes/source_data.html) for more details.
...
See the [sample data vignette](vignettes/sample_data.html) for more information including the built-in validations.
...
See the [feature data vignette](vignettes/feature_data.html) for more details.
...
The `get_comparison_groups()` function is entirely informational, but it is possible to pipe this output into the `run_comparison_groups()` function. This more advanced use in detailed in the [Multiple qSIP Objects vignette](vignettes/multiple_objects.html).
...
To determine if there are any differences in how a feature responds in different treatments, we can run the delta EAF workflow. This workflow is detailed in the [delta EAF vignette](vignettes/delta_EAF.html).
...
It is possible to work with multiple `qsip_data` objects if they are in a list. This is detailed in the [multiple objects vignette](vignettes/multiple_objects.html), but here is a sneak peak where we can use the existing `summarize_EAF_values()` or `plot_EAF_values()` functions.
...
This version of the workflow still requires running each comparison separately. A cleaner workflow, and the recommended route, is to define your comparisons upstream, even in an Excel file, and run that dataframe through the workflow. Details can be found in the [Multiple qSIP Objects vignette](vignettes/multiple_objects.html).

...
test_that("All values must be numeric or NA", {
  expect_error(calculate_resampled_wads(1, data.frame("A" = c(1.1,1.2,"not_a_number")), type = "labeled"),
               "ERROR: wad dataframe to resample from contains non-numeric dat")
  expect_equal(dim(calculate_resampled_wads(1, data.frame("A" = c(1.1,1.2,NA), "B" = c(2.1,2.2,2.3)), type = "labeled")),
               c(3,5))
})

...
get_shared_ids <- function(source_data, sample_data = NULL, feature_data = NULL) {
  ...
  if (isTRUE(setequal(source_source_mat_id, sample_source_mat_id))) {
    cli::cli_alert_success(
      "There are {length(source_source_mat_id)} source_mat_ids, and they are all shared between the source and sample objects."
    )
  } else {
  ...
  if (isTRUE(setequal(sample_sample_id, feature_sample_id))) {
    cli::cli_alert_success(
      "There are {length(sample_sample_id)} sample_ids, and they are all shared between the sample and feature objects."
    )
  } else {

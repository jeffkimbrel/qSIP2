...
plot_filter_gradient_position = function(qsip_data_object,
  ...
  if (is.null(colors)) {
    colors = c("Fraction Passed" = "#2B92BE",
  ...
  by_abundance = by_abundance_df |>
    ggplot2::ggplot(ggplot2::aes(x = source_mat_id,
  ...
  if (return_type == "combined") {
    patchwork::wrap_plots(A = (by_abundance +
